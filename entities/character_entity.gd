...
func receive_data(data: DataEntity, _soft = false): ##Used to load entity data (from a save file).
	if data:
		global_position = data.position
		facing = data.facing

func move_towards(_position, speed_increment = 1.0, friction_increment = 1.0): ##Moves the entity towards a position, with the possibility to modify speed and friction.
	var moving_direction = global_position.direction_to(_position)
	move(moving_direction, speed_increment, friction_increment)

func move(direction, speed_increment = 1.0, friction_increment = 1.0): ##Handles entity movement, applying the right velocity to the body.
...
func jump(): ##Starts a jump.
	if not is_jumping:
		safe_position = global_position
		is_jumping = true
		collision_mask ^= 1 << 2

func end_jump(): ##To be called at the end of a jump.
	is_jumping = false
	collision_mask ^= 1 << 2

func attack(): ##Starts an attack.
...
func end_attack(): ##To be called at the end of an attack.
	if attack_cooldown_timer:
			attack_cooldown_timer.start(attack_speed)

func flash(power := 0.0, duration := 0.1, color := Color.TRANSPARENT): ##Applies a flash to all children Sprite2D nodes found in group "flash" of the entity. 
...
func reduce_hp(value := 0, from = ""): ##Reduces entity hp.
	if from:
...
func recover_hp(value := 0, from = ""): ##Recovers entity hp.
	if from:
...
func hurt(): ##Useful to be called after reducing entity hp. IMPORTANT: should be called always after reduce_hp.
	if on_hurt:
		on_hurt.enable()

func add_impulse(force:= 0.0): ##Useful for dashing.
	velocity += facing * force

func return_to_safe_position(): ##Returns the entity to the latest safe position. safe_position is set before starting a jump. It is considered a non-safe position a position where the entity falls.
	if safe_position != Vector2.ZERO:
		global_position = safe_position

func consume_item(item: DataItem): ##Applies the effect of an item on the entity.
...
func disable_entity(value: bool, delay = 0.0): ##Stops the entity and disables its process.
